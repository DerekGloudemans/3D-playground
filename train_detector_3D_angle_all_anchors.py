...
detector_path = os.path.join(os.getcwd(),"pytorch_retinanet_detector_angle_anchors")
sys.path.insert(0,detector_path)
from pytorch_retinanet_detector_angle_anchors.retinanet.model import resnet50 
...
def plot_detections(dataset,retinanet):
    ...
    for bbox in boxes:
        ...
        xd/= length+1e-03
        yd/= length+1e-03
        cv2.line(cv_im,(bbox[0],bbox[1]),(int(bbox[0]+ 50*xd),int(bbox[1]+50*yd)),(0,1.0,0),thickness)
        ...
        xd/= length+1e-03
        yd/= length+1e-03
        cv2.line(cv_im,(bbox[0],bbox[1]),(int(bbox[0]+ 50*xd),int(bbox[1]+50*yd)),(1.0,0,0),thickness)
        ...
        xd/= length+1e-03
        yd/= length+1e-03
        cv2.line(cv_im,(bbox[0],bbox[1]),(int(bbox[0]+ 50*xd),int(bbox[1]+50*yd)),(0,0,1.0),thickness)
...
if __name__ == "__main__":
    ...
    max_epochs = 200
    start_epoch = 0
    checkpoint_file = None #"mutlitask__e62.pt"
    MULTI_GPU = False
    ...
    for epoch_num in range(start_epoch,max_epochs):
        ...
        for iter_num, (im,label) in enumerate(trainloader):
            ...
            try:
                ...
                loss = classification_loss + regression_loss + 10*vp_loss
        ...
        PATH = "mutlitask_angle_e{}.pt".format(epoch_num)
