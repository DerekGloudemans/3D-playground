...
class FocalLoss(nn.Module):
    ...
    def forward(self, classifications, regressions, anchors, annotations):
        ...
        # first, we will compute the angle-loss term for every anchor box output
        vps = annotations[:,0,21:27] # should be [b,6]
        annotations = annotations[:,:,:21]
        
        # expand vp and anchor dims
        vps = vps.unsqueeze(1).repeat(1,len(anchor_widths),1) # should be [b,a,6]
        vp_anchor_widths = anchor_widths.unsqueeze(0).unsqueeze(2).repeat(batch_size,1,3)
        vp_anchor_heights = anchor_heights.unsqueeze(0).unsqueeze(2).repeat(batch_size,1,3)
        vp_anchor_x = anchor_ctr_x.unsqueeze(0).unsqueeze(2).repeat(batch_size,1,3)
        vp_anchor_y = anchor_ctr_y.unsqueeze(0).unsqueeze(2).repeat(batch_size,1,3)
        
        # we'll need to convert the vanishing points first
        vps[:,:,[0,2,4]] = (vps[:,:,[0,2,4]] -vp_anchor_x ) / vp_anchor_widths
        vps[:,:,[1,3,5]] = (vps[:,:,[1,3,5]] -vp_anchor_y ) / vp_anchor_heights
        
        # regression has shape [b,a,20]
        obj_ctr_x = regressions[:,:,0]
        obj_ctr_y = regressions[:,:,1] 
        
    
        ### VP 1
        # we compute the line from each box towards each vp direction
        #vector components
        reg_vec_x = regressions[:,:,2]
        reg_vec_y = regressions[:,:,3]
        targ_vec_x = obj_ctr_x - vps[:,:,0]
        targ_vec_y = obj_ctr_y - vps[:,:,1]
        ...
        # we compute the line from each box towards each vp direction
        # vector components
        reg_vec_x = regressions[:,:,4]
        reg_vec_y = regressions[:,:,5]
        targ_vec_x = obj_ctr_x - vps[:,:,2]
        targ_vec_y = obj_ctr_y - vps[:,:,3]
        ...
        # we compute the line from each box towards each vp direction
        # vector_components        
        reg_vec_x = regressions[:,:,6]
        reg_vec_y = regressions[:,:,7]
        targ_vec_x = obj_ctr_x - vps[:,:,4]
        targ_vec_y = obj_ctr_y - vps[:,:,5]
        ...
        vp_loss = vp1_loss + vp2_loss + vp3_loss
        vp_loss = torch.mean(vp_loss)
        ...
        return torch.stack(classification_losses).mean(dim=0, keepdim=True), torch.stack(regression_losses).mean(dim=0, keepdim=True) ,vp_loss
