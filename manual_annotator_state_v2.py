...
class Annotator():
    ...
    def create_trajectory(self,idx,include_interp = True,y_order = 2, x_order = 3, plot = True,metric = "mpe"):
        """
        ...
        interp = torch.tensor([(0.1 if ("gen" in item.keys() and item["gen"] == "Interpolation") else 1) for item in boxes])
        ...
        for tpex in list(np.logspace(2,0,100)):
            x_spline = scipy.interpolate.fitpack2.UnivariateSpline(boxes[:,6],boxes[:,0],k = x_order,w=x_weight2,s = tpex**2*sum(interp))

            if np.isnan(x_spline(0)):
                 continue
            if len(x_spline.get_knots()) > np.floor(duration)+2:
                continue
    ...
    def get_splines(self,plot = False,metric = "mpe"):
        ...
        mpes = []
        for idx in range(self.get_unused_id()):
            t,x_spline,y_spline,ape,mpe = self.create_trajectory(idx,plot = plot,metric = metric)
    ...
    def adjust_ts_with_trajectories(self,max_shift = 0.01,trials = 21,overwrite_ts_data = False,metric = "mpe", use_running_error = False):
        splines = self.splines
        
        running_error = [0 for i in range(len(self.seq_keys))]
            ...
            for c_idx, cam in enumerate(self.seq_keys):
                ...
                for i in range(len(keep)):
                    if keep[i]:
                        ...
                        keep_splinesy.append(yid_splines[i])
                id_splines = keep_splines
                yid_splines = keep_splinesy
                
                boxes = boxes[keep]
                x_weight = x_weight[keep]
                y_weight = y_weight[keep]
                ...
                for shift in np.linspace(-max_shift,max_shift,trials):
                    ...
                    ys = torch.tensor([yid_splines[i](new_time).item() for i in range(len(yid_splines))])
                    ylabel = boxes[:,1]
                    
                    if metric == "ape":
                        mse = torch.mean(((xs-xlabel)*x_weight).pow(2)) # + ((ys-ylabel)*y_weight).pow(2))
                    else:
                        mse = torch.max(((xs-xlabel)*x_weight).pow(2))   # + ((ys-ylabel)*y_weight).pow(2))
...
if __name__ == "__main__":
    ...
    ann = Annotator(directory,scene_id = scene_id,homography_id = 2,exclude_p3c6 = exclude_p3c6)  
